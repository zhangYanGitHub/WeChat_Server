...
import com.zhang.chat.entity.response.NettyHeader;
...
import com.zhang.chat.netty.service.CheckToken;
...
public class ProtocolDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        if (in.readShort() != ProtocolHeader.MAGIC) {
            LogUtils.error(this.getClass(), in.readShort() + "   === " + ProtocolHeader.MAGIC);
            // Magic不一致，表明不是自己的数据
        ...
        NettyHeader header = Serializer.deserialize(s, NettyHeader.class);
        messageHolder.setBody(Serializer.serialize(header.getHeader()));
        CheckToken.newInstance().checkToken(messageHolder);


        messageHolder.setBody(header.getBody());
